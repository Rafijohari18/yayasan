...
<div class="container-fluid flex-grow-1 container-p-y">
    ...
    <h4 class="font-weight-bold py-3 mb-4">
      <span class="text-muted font-weight-light">Category Product / {{ ucfirst(Request::segment(2)) }} /</span> {{ $data['title'] }} <span class="text-muted"></span>
    </h4>

    <div class="card mb-4">
      <h6 class="card-header">
        <i class="fas fa-{{ $data['type'] == 'create' ? 'plus' : 'edit' }}"></i> {{ $data['type'] == 'create' ? 'Tambah Category Product' : 'Edit Category Product' }} 
      </h6>
      <div class="card-body">
        <form action="{{ $data['type'] == 'create' ? route('category.product.store') : route('category.product.update', ['id' => $data['content']['id']]) }}" method="POST">
            @csrf
            @if ($data['type'] == 'edit')
            @method('PUT')
            @endif
